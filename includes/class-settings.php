...
class Settings {
	...
	public static function format_interval( $seconds ) {
		...
		if ( $seconds < 60 ) {
			return sprintf(
				...
				_n( '%d second', '%d seconds', $seconds, 'projectmakers-health-endpoint' ),
				$seconds
			);
		}
		...
		return sprintf(
			...
			_n( '%d minute', '%d minutes', $minutes, 'projectmakers-health-endpoint' ),
			$minutes
		);
	}
	...
	public function add_menu() {
		add_menu_page(
			__( 'ProjectMakers Health Endpoint', 'projectmakers-health-endpoint' ),
			__( 'Health', 'projectmakers-health-endpoint' ),
			self::CAPABILITY,
			self::PAGE,
	...
	public function action_links( $links ) {
		...
		$settings  = '<a href="' . esc_url( $url ) . '">' . esc_html__( 'Settings', 'projectmakers-health-endpoint' ) . '</a>';
	...
	public function handle_test_email() {
		if ( ! current_user_can( self::CAPABILITY ) ) {
			wp_die( esc_html__( 'Insufficient permissions.', 'projectmakers-health-endpoint' ) );
		}
	...
	public function render_page() {
		...
		<div class="wrap he-wrap">
			<div class="he-header">
				...
				<div>
					<h1><?php esc_html_e( 'ProjectMakers Health Endpoint', 'projectmakers-health-endpoint' ); ?></h1>
					<p class="he-sub">
						<?php esc_html_e( 'Uptime endpoint & server monitoring', 'projectmakers-health-endpoint' ); ?>
					</p>
				</div>
				...
				<div class="notice notice-warning">
					<p>
						...
						printf(
							...
							esc_html__( 'No alert email is set; alerts fall back to the site admin address (%s). Set a dedicated address below.', 'projectmakers-health-endpoint' ),
			...
			<div class="he-grid">
				...
				<div class="he-card">
					<h2><?php esc_html_e( 'What this plugin does', 'projectmakers-health-endpoint' ); ?></h2>
					<p>
						<?php esc_html_e( 'Exposes a tiny public health endpoint for uptime monitors (Uptime Kuma & co.) that only confirms WordPress and the database respond; no sensitive data is ever revealed. Optionally it monitors the server internally (database, disk, CPU, RAM) and emails you when something breaches your thresholds.', 'projectmakers-health-endpoint' ); ?>
					</p>
				</div>

				<div class="he-card">
					<h2><?php esc_html_e( 'Live status', 'projectmakers-health-endpoint' ); ?></h2>
					<ul class="he-status">
						<?php
						$this->status_row( 'Database', $snapshot['db']['ok'], $snapshot['db']['ok'] ? __( 'connected', 'projectmakers-health-endpoint' ) . ' (' . (int) $snapshot['db']['latency_ms'] . ' ms)' : __( 'down', 'projectmakers-health-endpoint' ) );
					...
					<p class="he-muted">
						...
						if ( $last_run ) {
							...
							printf( esc_html__( 'Last internal check: %s ago', 'projectmakers-health-endpoint' ), esc_html( human_time_diff( $last_run ) ) );
							if ( null !== $last_duration_ms ) {
								echo '<br />';
								printf(
									/* translators: %s: formatted duration */
									esc_html__( 'Last check duration: %s', 'projectmakers-health-endpoint' ),
									esc_html( $this->format_duration( $last_duration_ms ) )
								);
							}
						} else {
							esc_html_e( 'Internal monitoring has not run yet.', 'projectmakers-health-endpoint' );
						}
						?>
					</p>
					<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="he-inline">
						...
						<button type="submit" class="button"><?php esc_html_e( 'Run check now', 'projectmakers-health-endpoint' ); ?></button>
					</form>
				</div>

				<div class="he-card he-card-wide">
					<h2><?php esc_html_e( 'Endpoints', 'projectmakers-health-endpoint' ); ?></h2>
					...
					<p class="he-muted">
						<?php esc_html_e( 'Healthy: HTTP 200 {"status":"ok"}. DB down: HTTP 503. Append a valid token for diagnostics:', 'projectmakers-health-endpoint' ); ?>
						<code>?token=...</code> <?php esc_html_e( 'or header', 'projectmakers-health-endpoint' ); ?> <code>X-Health-Token: ...</code>.
						<?php echo $token_set ? esc_html__( 'Diagnostics token is configured.', 'projectmakers-health-endpoint' ) : esc_html__( 'No token configured; diagnostics mode is off.', 'projectmakers-health-endpoint' ); ?>
					</p>
				</div>

				<div class="he-card he-card-wide">
					<h2><?php esc_html_e( 'Monitoring & alerts', 'projectmakers-health-endpoint' ); ?></h2>
					<form method="post" action="options.php">
						<?php settings_fields( self::GROUP ); ?>
						<table class="form-table" role="presentation">
							<tr>
								<th scope="row"><?php esc_html_e( 'Internal monitoring', 'projectmakers-health-endpoint' ); ?></th>
								<td>
									<label>
										<input type="checkbox" name="<?php echo esc_attr( self::OPTION ); ?>[monitoring_enabled]" value="1" <?php checked( $s['monitoring_enabled'], 1 ); ?> />
										<?php esc_html_e( 'Check the server at the configured interval and alert on breaches', 'projectmakers-health-endpoint' ); ?>
									</label>
									<p class="description"><?php esc_html_e( 'Runs via WP-Cron. For reliable checks on low-traffic sites, set up a real server cron (see README).', 'projectmakers-health-endpoint' ); ?></p>
								</td>
							</tr>
							<tr>
								<th scope="row"><label for="he-interval"><?php esc_html_e( 'Check interval', 'projectmakers-health-endpoint' ); ?></label></th>
								<td>
									...
									<p class="description"><?php esc_html_e( 'How often the internal database, disk, CPU, and RAM checks should run.', 'projectmakers-health-endpoint' ); ?></p>
								</td>
							</tr>
							<tr>
								<th scope="row"><label for="he-email"><?php esc_html_e( 'Alert email(s)', 'projectmakers-health-endpoint' ); ?></label></th>
								<td>
									...
									<p class="description"><?php esc_html_e( 'Comma-separated. Alerts are sent here when a check fails.', 'projectmakers-health-endpoint' ); ?></p>
								</td>
							</tr>
							<tr>
								<th scope="row"><?php esc_html_e( 'Disk threshold', 'projectmakers-health-endpoint' ); ?></th>
								<td>
									<?php $this->select_pct( 'disk_threshold', (int) $s['disk_threshold'], array( 60, 70, 80, 85, 90, 95 ) ); ?>
									<?php esc_html_e( 'used -> alert', 'projectmakers-health-endpoint' ); ?>
								</td>
							</tr>
							<tr>
								<th scope="row"><?php esc_html_e( 'CPU threshold', 'projectmakers-health-endpoint' ); ?></th>
								<td>
									<?php $this->select_pct( 'cpu_threshold', (int) $s['cpu_threshold'], array( 70, 80, 90, 100, 150, 200 ) ); ?>
									<?php esc_html_e( 'load-per-core, sustained for', 'projectmakers-health-endpoint' ); ?>
									<?php $this->number( 'cpu_minutes', (int) $s['cpu_minutes'], 1, 60 ); ?>
									<?php esc_html_e( 'minute(s)', 'projectmakers-health-endpoint' ); ?>
								</td>
							</tr>
							<tr>
								<th scope="row"><?php esc_html_e( 'RAM threshold', 'projectmakers-health-endpoint' ); ?></th>
								<td>
									<?php $this->select_pct( 'ram_threshold', (int) $s['ram_threshold'], array( 70, 80, 85, 90, 95 ) ); ?>
									<?php esc_html_e( 'used, sustained for', 'projectmakers-health-endpoint' ); ?>
									<?php $this->number( 'ram_minutes', (int) $s['ram_minutes'], 1, 60 ); ?>
									<?php esc_html_e( 'minute(s)', 'projectmakers-health-endpoint' ); ?>
								</td>
							</tr>
							<tr>
								<th scope="row"><?php esc_html_e( 'Repeat-alert cooldown', 'projectmakers-health-endpoint' ); ?></th>
								<td>
									<?php $this->number( 'alert_cooldown', (int) $s['alert_cooldown'], 0, 1440 ); ?>
									<?php esc_html_e( 'minute(s) between repeated alerts for the same issue', 'projectmakers-health-endpoint' ); ?>
									<p class="description"><?php esc_html_e( 'Set to 0 to send a single alert per incident (no reminders until it recovers).', 'projectmakers-health-endpoint' ); ?></p>
									<p>
										<label>
											<input type="checkbox" name="<?php echo esc_attr( self::OPTION ); ?>[recovery_email]" value="1" <?php checked( $s['recovery_email'], 1 ); ?> />
											<?php esc_html_e( 'Also send a recovery email when an issue clears', 'projectmakers-health-endpoint' ); ?>
										</label>
									</p>
								</td>
							</tr>
							<tr>
								<th scope="row"><label for="he-token"><?php esc_html_e( 'Diagnostics token', 'projectmakers-health-endpoint' ); ?></label></th>
								<td>
									<?php if ( $token_lock ) : ?>
										<input type="text" class="regular-text" value="<?php esc_attr_e( 'Defined in wp-config.php (HEALTH_ENDPOINT_TOKEN)', 'projectmakers-health-endpoint' ); ?>" disabled />
										<p class="description"><?php esc_html_e( 'The wp-config.php constant takes precedence over this field.', 'projectmakers-health-endpoint' ); ?></p>
									<?php else : ?>
										...
										<p class="description"><?php esc_html_e( 'Unlocks the diagnostics payload. Leave empty to keep diagnostics off. For best security define HEALTH_ENDPOINT_TOKEN in wp-config.php instead.', 'projectmakers-health-endpoint' ); ?></p>
										<p>
											<button type="submit" class="button" form="he-generate-token-form"><?php esc_html_e( 'Generate token', 'projectmakers-health-endpoint' ); ?></button>
										</p>
									<?php endif; ?>
								</td>
							</tr>
					...
					<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="he-inline">
						...
						<button type="submit" class="button"><?php esc_html_e( 'Send test email', 'projectmakers-health-endpoint' ); ?></button>
					</form>
				</div>
			...
			<div class="he-footer">
				<em>
					<?php esc_html_e( 'Made with', 'projectmakers-health-endpoint' ); ?>
					<span class="he-heart" aria-label="<?php esc_attr_e( 'love', 'projectmakers-health-endpoint' ); ?>">&hearts;</span>
					<?php esc_html_e( 'by', 'projectmakers-health-endpoint' ); ?>
	...
	private function render_notice() {
		...
		$map = array(
			'testmail_ok'     => array( 'updated', __( 'Test email sent.', 'projectmakers-health-endpoint' ) ),
			'testmail_fail'   => array( 'error', __( 'Test email could not be sent. Check the alert address and your mailer (e.g. WP Mail SMTP).', 'projectmakers-health-endpoint' ) ),
			'ran'             => array( 'updated', __( 'Internal check executed.', 'projectmakers-health-endpoint' ) ),
			'token_generated' => array( 'updated', __( 'Diagnostics token generated.', 'projectmakers-health-endpoint' ) ),
			'token_locked'    => array( 'error', __( 'Diagnostics token is defined in wp-config.php and cannot be generated here.', 'projectmakers-health-endpoint' ) ),
		);
		if ( ! isset( $map[ $notice ] ) ) {
			return;
		}
	...
	private function metric_row( $label, $metric, $key, $threshold ) {
		if ( null === $metric ) {
			$this->status_row( $label, true, __( 'n/a on this host', 'projectmakers-health-endpoint' ) );
			return;
		}
	...
	private function format_duration( $milliseconds ) {
		...
		if ( $milliseconds < 1000 ) {
			return sprintf(
				...
				_n( '%d ms', '%d ms', $milliseconds, 'projectmakers-health-endpoint' ),
				$milliseconds
			);
		}
		...
		return sprintf(
			...
			__( '%.2f s', 'projectmakers-health-endpoint' ),
			$milliseconds / 1000
		);
	}
}
